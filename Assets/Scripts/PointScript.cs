...
public class PointScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (received != "")
        {
            ...
            //}
            //Clear message
            compass.text = splitted[0];
            lidar.text = splitted[1];
            humid.text = splitted[2];
            temp.text = splitted[3];
            OX = float.Parse( splitted[4]);
            OY = float.Parse(splitted[5]);
            OZ = float.Parse(splitted[6]);
